...
const App = () => {
  ...
  const [modelSelection, setModelSelection] = useState(
    MAHALANOBIS_DISTANCE_CLASSIFIER
  );
  ...
  const getROIdata = (name = "") => {
    ...
    if (storedData) {
      const newData = Object.keys(storedData).map((ele, index) => (
        <option key={index} value={storedData[ele] || ""} name={ele}>
          {ele}
        </option>
      ));
      setROIdata(newData);
  ...
  const generateImageFromPixels = useCallback(
    (imageURLFromBackend, geoJson) => {
  ...
  const sendGeoJsonData = async () => {
    try {
      ...
      const eventSource = new EventSource(
        "http://127.0.0.1:5001/stream_images",
        {
          withCredentials: true,
        }
      );
